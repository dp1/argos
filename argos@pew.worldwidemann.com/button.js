...
export let ArgosButton = GObject.registerClass({
...
class ArgosButton extends PanelMenu.Button {
        ...
        (standardOutput) => {
          ...
          if (this._updateInterval !== null) {
            this._updateTimeout = GLib.timeout_add_seconds(
              GLib.PRIORITY_DEFAULT_IDLE, this._updateInterval,
              () => {
                this._updateTimeout = null;
                this._update();
                return false;
  ...
  _processOutput(output) {
    ...
    if (buttonLines.length === 0) {
      ...
      this._lineView.setLine(buttonLines[0]);
      let i = 0;
      this._cycleTimeout = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT_IDLE, 3, () => {
        i++;
